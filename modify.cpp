...
bool ModifyFirmware(enModel model)
{
	...
	nPosVHeightCheck2 = -1;
	if (model == P2214H_P72WF || model == P2314H_48H1R || model == P2314H_79H3D) {	// P2214H/P2314Hはこちら
...
void OutputISTPTR(enModel model, int &nOffset)
{
	...
	else if (model == EK271Ebmix_2 || model == EK271Ebmix_3) {
...
{
	if (model == EK241YEbmix || model == EK271Ebmix || model == EK271Ebmix_2 || model == EK271Ebmix_3 || model == EK221QE3bi) {
...
bool AddAspectModeForDell(enMode mode, enModel model)
{
	...
	BYTE	bAddr[2];
	if (model == P2214H_P72WF) {	// P2214H/P2314Hはこちら
		nFuncOffset = 0x1FA00;
		nCallOffset = 0x1F9B1;
